...
type GitHubFollowersButtonProps = Omit<HTMLMotionProps<'a'>, 'ref'> & {
  username: string;
...
function GitHubFollowersButton({
  ...
  useEffect(() => {
    if (followers > 0) motionVal.set(followers);
  }, [motionVal, followers]);

  const fillPercentage = followers
    ? Math.min(100, (motionNumberRef.current / followers) * 100)
    : 0;
  ...
  return (
    <motion.a
      ...
      aria-label={`${username}'s GitHub profile (followers)`}
      {...props}
    >
...
export { GitHubFollowersButton, type GitHubFollowersButtonProps };
